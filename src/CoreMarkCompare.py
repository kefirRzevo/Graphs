...
class TestResult:
    def __init__(self, regalloc: str, virt_regs_count: int, virt_regs_num: int, phys_regs_num: int):
...
def draw_results(packs: list[TestPack]):
    random.seed(0)
...
def main():
    pack_names = ["core_list_join.ll", "core_main.ll", "core_matrix.ll", "core_portme.ll", "core_state.ll", "core_util.ll"]
    regallocs = ["pbqp", "custom", "greedy"]
    ...
    for pack_name in pack_names:
        ...
        for regalloc in regallocs:
            command = f"{llvm_repo_path}/build/bin/llc -mtriple=riscv64-unknown-linux-gnu -regalloc {regalloc} {llvm_repo_path}/coremark/coremark/{pack_name}"
            output = run_command(command)
            matches = re.findall(r"Virt regs count: (\d+)\nName: (.+?)\nVirt regs num: (\d+)\nPhys regs num: (\d+)", output)
            for match in matches:
                ...
                test_result = TestResult(regalloc, virt_regs_count, virt_regs_num, phys_regs_num)
                found = False
